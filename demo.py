...
def fun_rec():
    global is_recording
    is_recording = True  # Set recording state to True
    ...
    while my_temp > 0 and is_recording:
    ...
    if is_recording:
        ...
        psound.stop()  # Stop recording immediately if "Stop" is pressed
        messagebox.showinfo("Recording stopped", "Recording has been stopped manually")

# Function to stop recording
def stop_rec():
    global is_recording
    is_recording = False  # Set recording state to False
...
stop_bt = Button(window, font=("Arial", 22, "bold"), text="Stop", bg="red", fg="black", borderwidth=0, command=stop_rec).pack()
